...
public class CPU_scheduling {
    ...
    // first come first serve CPU scheduling algorithm at = is for arrival time, bt= burst time, pc= number of processes (process count)
    void fcfs(int at[], int bt[], int pc) {
        int i, j, k = 0, min, head = 0;

        // ready_queue keeps the index of processes in decresing order .i.e first in the
        // queue in on last position on ready_queue
        int[] ready_queue = new int[pc];
        int[] array = new int[pc];
        array = at.clone();

        // this is for entering order of process based on their arrival time in
        // ready_queue
        for (i = pc - 1; i >= 0; i--) {
            min = array[i];
            for (j = 0; j < pc; j++) {
    ...
    // shortest job first non premptive cpu scheduling at = is for arrival time, bt= burst time, pc= number of processes (process count)
    ...
    // priority scheduling algorithm at = is for arrival time, bt= burst time, pc= number of processes (process count)
    ...
    void priority_scheduling_p(int at[], int bt[], int pc, int priority[]) {
        ...
        while (no_of_p_complete != pc) {
            ...
            if (remaining_time[current_process_id] == 0) {
                ...
                // Calculating waiting time for the completed process
                waiting_time[current_process_id] = completion_time[current_process_id] - bt[current_process_id]
                        - at[current_process_id];
    ...
    void print_data(int at[], int bt[], int turn_around_time[], int completion_time[], int waiting_time[],
            int ready_queue[], int pc) {
        int i, j;
        ...
        for (i = 0; i < pc; i++) {
            ...
            System.out.println("|\tP" + (j + 1) + "\t|\t" + at[j] + "\t|\t" + bt[j] + "\t|\t" + completion_time[j]
                    + "\t|\t" + turn_around_time[j] + "\t|\t" + waiting_time[j] + "\t|");
    ...
    void print_data_for_priorty(int at[], int bt[], int turn_around_time[], int completion_time[], int waiting_time[],
            int process_id[], int priority[], int pc) {
        int i;
        int average_turn_around_time = 0;
        int average_waiting_time = 0;
        System.out.println("\t\tALL TIMES ARE IN milliseconds (ms)");
        ...
        System.out.println(
                "+---------------+----------+---------------+---------------+-----------------+-----------------+--------------+");
        for (i = 0; i < pc; i++) {
            System.out.println("|\tP" + (process_id[i] + 1) + "\t|    " + (priority[i] + 1) + "     |\t" + at[i]
                    + "    \t|\t" + bt[i] + "\t|\t" + completion_time[i] + "\t|\t" + turn_around_time[i] + "\t|\t"
                    + waiting_time[i] + "\t|");
